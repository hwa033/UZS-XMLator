...
from web.app import app, list_generated_files, get_output_directory
...
def test_missing_bsn():
    ...
    with open(path, 'rb') as f:
        resp = client.post('/upload_excel', data={
...
def test_path_traversal():
    ...
    client = app.test_client()
    resp = client.post('/resultaten/delete-selected',
...
def test_concurrent_delete():
    ...
    # First delete
    resp1 = client.post('/resultaten/delete-selected',
        json={'filenames': [filename]},
        headers={'Content-Type': 'application/json'})
    
    result1 = resp1.get_json()
    print(f'First delete - Status: {resp1.status_code}, Success: {result1.get("success")}')
    
    # Second delete (file already gone)
    resp2 = client.post('/resultaten/delete-selected',
        json={'filenames': [filename]},
        headers={'Content-Type': 'application/json'})
    
    result2 = resp2.get_json()
    print(f'Second delete - Status: {resp2.status_code}, Success: {result2.get("success")}')
...
def test_upload_json_invalid():
    ...
    client = app.test_client()
    resp = client.post('/genereer_xml_json/upload_json', data={
