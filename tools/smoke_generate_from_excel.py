...
rows, formula_count = gen.read_excel_rows(str(repo_root / 'docs' / 'Input XML electr ziekmeldinge.xlsx'), data_only=True)
...
bodies, errors, bulk = validate_normalized_rows_for_generator(norm_rows, list(rows[0].keys()) if rows else [], False, 'VM', {'Digipoort':'OTP3'}, 'VM', gen, ns_body, schema=None)
print('Errors:', errors)
